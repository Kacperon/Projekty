...
class Kółko:
    def __init__(self):
        self.tab = [[] for _ in range(25)]
        self.i = 0
        self.j = 0

    def put(self, arg):
        distance, v, time = arg
        index = (self.i + distance % 24) % 24
        self.tab[index].append((distance, v, time))

    def get(self):
        while not self.tab[self.i]:
            self.i = (self.i + 1) % 24
        item = self.tab[self.i][self.j]
        self.j += 1
        if self.j >= len(self.tab[self.i]):
            self.tab[self.i] = []
            self.i = (self.i + 1) % 24
            self.j = 0
        return item
    ...
    def empty(self):
        return all(len(slot) == 0 for slot in self.tab)
